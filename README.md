...
Instead of buying expensive proprietary hardware, workers simply open a local web link on their phones, scan a product, enter the quantity (In/Out), and the system instantly updates the master centralized database in real-time.
...
* **🧠 Stateful Inventory Logic:** Automatically matches scanned barcodes to existing records, preventing duplicates and dynamically updating the total stock available in a clean, easy-to-read master dashboard.
* **🛡️ Data Sanitization:** Includes built-in parsing logic to safely handle mixed data types and "leading zero" barcode anomalies common in Excel data pipelines.
...
To demo this software, host it on your local machine and access it via a smartphone. 
